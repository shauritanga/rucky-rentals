      ...
      {tab === 'analytics' && (
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            ...
            <div className="card"><div className="card-header"><div className="card-title">Monthly Trend</div></div><div style={{padding:'0 8px 12px'}}><div style={{display:'flex',alignItems:'flex-end',gap:8,height:100,padding:'12px 12px 0',borderBottom:'1px solid var(--border-subtle)'}}>{(() => { const months=['Oct','Nov','Dec','Jan','Feb','Mar']; const costs=[340000,520000,180000,620000,290000,normalizedTickets.reduce((s,r)=>s + Number(r.total_cost || 0),0)]; const maxCost=Math.max(...costs,1); return months.map((m,i)=><div key={m} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:4}}><div style={{fontSize:10,color:'var(--text-muted)'}}>{formatCompactTzsFromUsd(costs[i])}</div><div style={{width:'100%',background:i===5?'var(--accent)':'var(--accent-dim)',borderRadius:'4px 4px 0 0',height:`${Math.round((costs[i]/maxCost)*70)}px`,minHeight:4}} /><div style={{fontSize:11,color:'var(--text-muted)'}}>{m}</div></div>); })()}</div></div></div>
          </div>
        </>
      )}
